...
## ⚡ Авторасчет через "Рассчитать позицию"

Кнопка **"Рассчитать позицию"** открывает модальное окно, в котором можно автоматически заполнить входы (Multiple Entry) или выходы (Multiple Exit) по заданному риску.

### Что задается в модальном окне
- **Риск при стоп-лоссе (USDT)** — целевой риск, к которому подгоняется итоговая позиция
- **Количество точек** — от 2 до 10
- **Диапазон цен "От/До"** — границы сетки с проверками для Long/Short
- **Способ распределения**:
  - ручной ввод процентов (сумма должна быть 100%)
  - линейно по коэффициенту (слайдер от 0.1 до 2.0)
- **Стоп-лосс** и **Тейк-профит / Цена входа** — синхронизируются с полями в "Настройках позиции"

### Что делает расчет
- Равномерно интерполирует цены между "От" и "До"
- Строит распределение объемов по выбранному способу
- Заполняет таблицу входов/выходов автоматически
- Стремится попасть в целевой риск с отклонением не более 3%

Черновик формы сохраняется в браузере и подставляется при повторном открытии модального окна.
...
**Сохранение:** Пресеты автоматически сохраняются в браузере (localStorage) и подставляются при следующем заходе на сайт — настраивать их заново не нужно.
...
Показывает данные только по **заполненным и валидным** точкам входа (у которых указаны «Цена входа» и «Сумма (USDT)» и цена входа находится в допустимом диапазоне между SL и TP):

- **Средняя цена** - взвешенная средняя цена по валидным входам
- **Общий объем** - суммарное количество актива по валидным входам
...
Невалидные строки (например, с ценой входа вне диапазона TP–SL) **не учитываются** в сводке. Блок отображается, когда есть хотя бы одна рассчитанная точка входа. При добавлении новой (пустой) точки входа сводка не обнуляется: в ней по-прежнему отображаются данные по уже заполненным входам до тех пор, пока вы не заполните новую строку.
...
Если по точке входа показывается предупреждение (например, «Цена входа должна быть между TP и SL»):
- **Расчёт для этой строки не выполняется** — в ячейках средней цены, объёма, % до SL/TP, PnL и R/R отображаются пустые значения (как до расчётов). Заполненными остаются только столбцы «Цена входа» и «Сумма USDT».
- Эта точка входа **не участвует в сводке по позиции** и не влияет на расчёт других строк. Исправьте цену входа или SL/TP, чтобы строка стала валидной и попала в расчёты.
