...
Показывает данные по **заполненным** точкам входа (у которых указаны «Цена входа» и «Сумма (USDT)»):

- **Средняя цена** - взвешенная средняя цена по заполненным входам
- **Общий объем** - суммарное количество актива по заполненным входам
...
Блок отображается, когда есть хотя бы одна рассчитанная точка входа. При добавлении новой (пустой) точки входа сводка не обнуляется: в ней по-прежнему отображаются данные по уже заполненным входам до тех пор, пока вы не заполните новую строку.
